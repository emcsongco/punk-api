...
describe('Search Box', () => {
  it('should show the Search for Beers label', () => {
    render(<App />);
    const searchBeersText = screen.getByText("Search...");
...
test('renders second filter by name', ()=> {
  render(<App/>);
  const navigation = screen.getByText("Classic Range (< 2010)");
  expect(navigation).toBeInTheDocument();
} )
...
test('renders #2 checkbox by name', ()=> {
  render(<App/>);
  const myCheckbox = screen.getByRole('checkbox', {name: "Classic Range (< 2010)"});
  expect(myCheckbox).toBeInTheDocument();
} )
...
test('renders Search', ()=> {
  ...
  // expect(mySearch).toHaveAttribute("label","Search...")
  expect(mySearch).toHaveAttribute("value", "")
} )

test('click on 3 checkboxes and check epmty filter', ()=> {
  render(<App/>);
  const myFirstCheckbox = screen.getByRole('checkbox', {name: "High ABV > 6%"});
  const mySecondCheckbox = screen.getByRole('checkbox', {name: "Classic Range (< 2010)"});
...
test("should search for Pilsner", async () => {
  ...
  await waitFor(() => expect(screen.getByText("Buzz")).toBeInTheDocument(), {timeout: 2000});
  const mySearch = screen.getByRole('textbox');
  userEvent.type(mySearch, "ip");
  expect(screen.getByText("Dana - IPA Is Dead")).toBeInTheDocument()
});
